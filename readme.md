...
есть нескоько полезных методовюкоторые помогают хорошо дебажить
()окно expression evaluation Alt+F8)
первый -getBeanDefinitionNames()
оттого что поставили config, появилось много бинов- autoweiredAnnotatonionBeanProce

        стоп, у Борисова упало с эксепшеном NoSuchBeanDef... 
        а у меня совсем другое, не удается создать бин. 
        вложеный эксепшн - NPE
        поищем,где сломалось.... 
    ...забыл проинициализиорлвать new HashMap<>()

    Кстати, а проблемы как у Борисова, у меня не возникло - вытаскиваем по классу, и всё норм.
    А надо бы по интерфейсу 
        ^ разобраться, что он имел в видупод словом "вытаскивать": в main() или в xml?^

теперь разбираемся с MBean
Борисов идет в bin/jvisualvm
а я ходил в jconsole 
надо найти и запустить jconsole из пакета java

...
не работало, т.к забыл поставить аннотацию @Profiling
поcтавил на класс TerminatorQuoter, все сломалось на вызове метода прокси
поставлю на метод))

...^теперь не работает, т.к в InvocationHandler я вызываю method.invoke на переданный в него proxy,а надо бы на изначальный bean
